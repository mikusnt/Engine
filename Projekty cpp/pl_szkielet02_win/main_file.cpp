...
void initOpenGLProgram(GLFWwindow* window) {
    ...
    vec3 temp;
	 for(int i = 0; i < cnt; i++) {
        do {
                point[(i*3)+0] = ((rand() % 1000) -500)/ 1000.0f;
                point[(i*3)+1] = ((rand() % 1000) -500)/ 1000.0f;
                point[(i*3)+2] = ((rand() % 1000) -500)/ 000.0f;

                temp = vec3(point[i*3], point[(i*3)+1], point[(i*3)+2]);
...
void drawScene(GLFWwindow* window,float angle) {
   ...
   M=rotate(M,3.0f,vec3(0.0f,1.0f,0.0f));
    ...
    //glColor3d(abs(cos(angle)),abs(sin(angle)),0.0f); //Ustaw kolor rysowania
